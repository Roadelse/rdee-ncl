...
#@ Introduction
###############################################################################
# This scripts aims to deploy the rdee-ncl in system                          #
# Support flexible deployment method, inlcuding:                              #
#    ● setenv bash script                                                     #
#    ● modulefile                                                             #
# ----------------------------------------------------------------------------#
#                                                                             #
# 2024-04-18  Roadelse  Initialized                                           #
###############################################################################
...
#@ <.pre-check>
#@ ..ncl
if [[ -z $(which ncl 2>/dev/null) ]]; then
    warning "Cannot find ncl"
fi
#@ <..python3>
...
if [[ $utest == 0 ]]; then
    ...
    ln -sf $scriptDir/../src/bin/ncl* .
    cd $script_dir

    text_setenv="# >>>>>>>>>>>>>>>>>>>>>>>>>>> [$proj]
export PATH=${scriptDir}/export/bin:\$PATH
export NCL_RDEE=${scriptDir}/../src
